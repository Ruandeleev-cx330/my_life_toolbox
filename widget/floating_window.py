...
class FloatingWindow:
    ...
    def _build_ui(self):
        ...
        self._ip_icon = tk.Label(
            self._ip_card,
            text="🖥",
            bg=BG_CARD,
            font=("Segoe UI", 12),
        )
        self._ip_icon.pack(side="left", padx=(0, 5))
        ...
        self._ip_label.pack(side="left", fill="x", expand=True)
        self._ip_label.bind("<Button-1>", _on_ip_click)

        self._ip_copy_btn = tk.Label(
            self._ip_card,
            text="📋",
            fg=FG_SECONDARY,
            bg=BG_CARD,
        ...
        self._ip_copy_btn.pack(side="right")
        self._ip_copy_btn.bind("<Button-1>", lambda e: self._copy_ip())

        # 切换内外网标签
        self._ip_switch_label = tk.Label(
            self._ip_card,
            text="",
            fg=FG_SECONDARY,
            bg=BG_CARD,
        ...
        self._ip_switch_label.pack(side="right", padx=(0, 4))
    ...
    def _refresh_data(self):
        ...
        # 网络数据在后台线程刷新
        def _fetch_network():
            self._weather_text = fetch_weather()
            self._internal_ip = get_internal_ip()
            self._external_ip = get_external_ip()
            # 回到主线程更新 UI
    ...
    def _update_display(self):
        """将缓存数据刷新到 UI 组件"""
        # 天气
        self._weather_label.configure(text=self._weather_text)
    ...
    def _update_ip_display(self):
        """更新 IP 标签显示"""
        if self._ip_index == 0:
            ip = self._internal_ip or "--"
            label = f"内网: {ip}"
            self._ip_switch_label.configure(text="⇄ 外网")
        else:
            ip = self._external_ip or "--"
            label = f"外网: {ip}"
            self._ip_switch_label.configure(text="⇄ 内网")
        self._ip_label.configure(text=label)
        self._ip_icon.configure(text="🖥" if self._ip_index == 0 else "🌐")
    ...
    def _copy_ip(self):
        """复制当前 IP 到剪贴板"""
        ip = self._internal_ip if self._ip_index == 0 else self._external_ip
        if ip and ip != "--" and ip != "获取失败":
            self.root.clipboard_clear()
            self.root.clipboard_append(ip)
            self._ip_copy_btn.configure(text="✅")
            self.root.after(1500, lambda: self._ip_copy_btn.configure(text="📋"))
    ...
    def _open_quick_add(self):
        ...
        category_entry.pack(fill="x", ipady=2, pady=(0, 8))
